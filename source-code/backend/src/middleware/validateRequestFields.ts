...
export function validateUserFields(enablePartialDto: boolean = false) {
    return (req: Request, res: Response, next: NextFunction) => {
        ...
        if (!isUndefined(sentUser.rolename) && !isRolenameValid(sentUser.rolename)) {
            throw new createError.BadRequest("Role name is not valid.");
        }
...
export function validateSightingFields(enablePartialDto: boolean = false) {
    return (req: Request, res: Response, next: NextFunction) => {
        ...
        // Only add photoUrl key if a file was actually uploaded; subsequent code can then distinguish "not provided" (key absent) from "explicitly set to undefined" via the "in" operator
        if (res.locals.photoUrl !== undefined) {
            sentSighting.photoUrl = res.locals.photoUrl;
        }

        // Check required request fields
        if (!enablePartialDto) {
            if (isUndefined(sentSighting.photoUrl)) {
                throw new createError.BadRequest("Photo must be provided.");
            }
            ...
            if (isUndefined(sentSighting.username)) {
                throw new createError.BadRequest("Username must be provided.");
            }
        }
        ...
        if (!isUndefined(sentSighting.photoUrl) && !isPhotoUrlValid(sentSighting.photoUrl)) {
            throw new createError.BadRequest("Photo is not valid.");
        }
        ...
        if (!isUndefined(sentSighting.username) && !isUsernameValid(sentSighting.username)) {
            throw new createError.BadRequest("Username is not valid.");
        }
    ...
    return (req: Request, res: Response, next: NextFunction) => {
        ...
        if (!enablePartialDto) {
            if (isUndefined(sentComment.content)) {
                throw new createError.BadRequest("Content must be provided.");
            }
            if (isUndefined(sentComment.username)) {
                throw new createError.BadRequest("Username must be provided.");
            }
            if (isUndefined(sentComment.sightingId)) {
                throw new createError.BadRequest("Sighting ID must be provided.");
            }
        }
        ...
        if (!isUndefined(sentComment.username) && !isUsernameValid(sentComment.username)) {
            throw new createError.BadRequest("Username is not valid.");
        }
        if (!isUndefined(sentComment.sightingId) && !isIdValid(sentComment.sightingId)) {
            throw new createError.BadRequest("Sighting ID is not valid.");
        }
